...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut request = SearchRequest::new("한국외국어대학교".to_string()); // 검색어
    //request.limit = Some(5);
    //request.safe_search = Some(true);
    // request.lens_id = Some("TT8mzzN1G5jHaV9ih0lH6SlMSbru5RGW".to_string()); // kagi 렌즈
    request.filters = Some(Box::new(SearchRequestFilters { // 검색 결과 필터
        region: Some("kr".to_string()), // 소문자로 된 국가 코드
        after: None,
        before: None,
    }));
    request.page = Some(2);
    // Note: 렌즈를 직접 설정하는 건 작동하지 않는 것으로 보임
